...
### 1.5 Listar Tiendas del Usuario (logueado)
Devuelve todas las tiendas a las que el usuario autenticado tiene acceso. Usa el **token final**.

- **URL:** `GET /api/v1/auth/shops`
- **Auth:** Requiere token final en `Authorization`.
- **Body:** No aplica.
- **Respuesta esperada:**
```json
{
...
```
- **Frontend:** Se usa para mostrar el selector de cambio de tienda dentro del panel.

### 1.6 Cambiar de Tienda (switch)
Permite a un usuario logueado cambiar de tienda sin cerrar sesión. El backend valida que el usuario tenga acceso a la tienda solicitada y devuelve un **nuevo token final** con el `shop_id` y rol actualizados.

- **URL:** `POST /api/v1/auth/switch-shop`
- **Auth:** Requiere token final en `Authorization`.
- **Body (JSON):**
```json
{
...
```
- **Respuesta:** devuelve `token` final nuevo y datos del usuario.
- **Frontend:** reemplaza el token actual por el nuevo y recarga la página para cargar datos de la nueva tienda.
...
### 2.0 Tienda Actual (propio)

#### Obtener Tienda
Obtiene la configuracion de la tienda autenticada.

- **URL:** `GET /api/v1/shop`
- **Auth:** Requiere token final.
- **Body:** No aplica.
- **Respuesta:** datos de la tienda (`name`, `email`, `phone`, `address`, `description`, `currency`, `timezone`, `slug`, `is_active`).

#### Actualizar Tienda
Actualiza parcialmente la configuracion de la tienda autenticada.

- **URL:** `PATCH /api/v1/shop`
- **Auth:** Requiere token final.
- **Roles:** `owner`, `admin`.
- **Body (JSON):**
```json
{
  "name": "Mi Tienda Renovada",
...
Sube o reemplaza el logo de la tienda autenticada. El archivo se guarda en Supabase Storage bucket `shop-logos` y la URL publica se almacena en `shops.logo_url`.

- **URL:** `PATCH /api/v1/shop/logo`
- **Auth:** Requiere token final.
- **Roles:** `owner`, `admin`.
...
```json
{
  ...
  "data": {
    "id": "uuid",
    "name": "Mi Tienda",
    "logo_url": "https://.../shop-logos/uuid-logo.png",
    "slug": "mi-tienda",
...
```
- **Notas frontend:**
  - Usar `FormData` con `append('logo', file)`.
  - Axios setea automaticamente `Content-Type: multipart/form-data` con boundary si no se fuerza header manualmente.
  - Despues de subir, refrescar `GET /shop` o usar directamente la respuesta para mostrar el logo en Sidebar.

#### Eliminar Tienda (desactivar)
Desactiva la tienda autenticada con soft delete. Solo el `owner` puede ejecutar esta accion.

- **URL:** `DELETE /api/v1/shop`
- **Auth:** Requiere token final.
- **Roles:** `owner`.
- **Body:** No aplica.
- **Respuesta si tiene otras tiendas:**
```json
{
  "success": true,
  "data": {
    "deleted_at": "2026-05-10T20:00:00.000Z",
    "token": "nuevo-jwt-final",
    "shop": {
      "shop_id": "uuid",
      "shop_name": "Otra Tienda",
      "shop_slug": "otra-tienda",
      "role": "owner"
    }
  }
}
```
- **Respuesta si era la ultima tienda:**
```json
{
  "success": true,
  "data": {
...
```
- **Notas:**
  - El backend busca automaticamente otra tienda activa del usuario.
  - Si encuentra una, genera un nuevo token final para esa tienda y lo devuelve.
  - Si no quedan tiendas, el usuario debe cerrar sesion manualmente.
...
### 2.5 Subir Imagen de Categoria
Sube o reemplaza la imagen de una categoria especifica. El archivo se guarda en Supabase Storage bucket `category-images` bajo la ruta `${shop_id}/${Date.now()}-${nombreOriginal}`. La URL publica se almacena en `categories.image_url`.
...
```json
{
  ...
  "data": {
    ...
    "slug": "bebidas",
    "description": "Productos liquidos",
    "image_url": "https://.../category-images/shop-uuid/1234567890-foto.jpg",
    "is_active": true,
    "parent_id": null,
    "created_at": "2026-05-09T20:00:00.000Z",
    "updated_at": "2026-05-09T20:00:00.000Z"
...
```
- **Notas frontend:**
  - Usar `FormData` con `append('image', file)`.
  - Este endpoint es independiente de `PATCH /categories/:id`; se puede usar para cambiar solo la imagen.
  - Despues de subir, refrescar `GET /categories` para sincronizar la URL.
...
### 2.13 Listar Ofertas
Lista ofertas y promociones de la tienda autenticada.
...
- **Query params:**
  - `is_active`: `true` o `false`.
  - `scope`: `storewide`, `product`, `category`.
  - `page`: default `1`.
  - `limit`: default `20`, maximo `100`.
- **Respuesta:** lista paginada de ofertas con `product_name` y `category_name` resueltos.
...
### 2.15 Crear Oferta
Crea una promocion o descuento.

- **URL:** `POST /api/v1/offers`
- **Auth:** Requiere token.
- **Roles:** `admin`, `owner`.
- **Body (JSON):**
```json
...
- **Validaciones:**
  ...
  - `discount_value`: si es `percentage`, maximo `100`.
  - `scope`: `storewide`, `category` o `product`.
  - Si `scope = product` -> obligatorio `product_id`.
  - Si `scope = category` -> obligatorio `category_id`.
  - Si `scope = storewide` -> no enviar `product_id` ni `category_id`.
  - `ends_at` debe ser posterior a `starts_at`.
  - `code` es opcional, unico por tienda, se guarda en mayusculas.

### 2.16 Actualizar Oferta
Actualiza parcialmente una oferta.

- **URL:** `PATCH /api/v1/offers/:id`
- **Auth:** Requiere token.
- **Roles:** `admin`, `owner`.
- **Body (JSON):**
```json
{
  "title": "Verano 2025 Extendido",
  "ends_at": "2025-09-15T23:59:59Z",
  "is_active": false
}
```

### 2.17 Eliminar Oferta (desactivar)
Desactiva una oferta con soft delete.
